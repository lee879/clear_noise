...
def main():
    ...
    for epoch in range(epochs):
        ...
        patch_img = images_to_patches(img,patch_size)
        for i in range(patch_img.shape[0]):
            # noise_scal_ = noise_scal[np.random.randint(low=0, high=len(noise_scal))] / 255.0
            img_noise.append(gaussian_noise(patch_img[i]))
        img_noise = np.array(img_noise) / 255. # input 数据
        #img_noise_input = downsample(img_noise)

        for i in range(patch_img.shape[0]):
            #noise_scal_ = noise_scal[np.random.randint(low=0, high=len(noise_scal))] / 255.0
            img_noise_target.append(gaussian_noise(patch_img[i]))
       ...
       #Y_P = np.array(upsample(y_p))
        ...
        with summary_writer.as_default():
            ...
            if epoch % 100 == 0:
                ...
                tf.summary.image("real_image", patch_img[:3]/255., step=epoch)
